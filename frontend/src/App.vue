  ...
  <div v-else-if="!auth || !auth.user" class="w-full h-screen grid place-items-center bg-gradient-to-br from-emerald-50 to-teal-100">
    ...
    <div v-if="showForgotPassword" class="fixed inset-0 bg-black bg-opacity-50 grid place-items-center z-[100]" @click.self="showForgotPassword = false">
      <div class="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 mx-4">
        ...
        <form v-if="!forgotSuccess" class="space-y-4" @submit.prevent="sendResetEmail">
          <input v-model="forgotEmail" class="w-full border rounded-xl px-3 py-2" placeholder="Email" />
          <p v-if="forgotError" class="text-sm text-red-600">
            {{ forgotError }}
          </p>
          <div class="flex gap-2">
            <button type="button" class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-xl py-2 font-medium" @click="showForgotPassword = false">
              Annuler
            </button>
            <button type="submit" :disabled="forgotLoading" class="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl py-2 font-medium">
        ...
        <div v-else class="text-center">
          ...
          <button class="w-full bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl py-2 font-medium" @click="showForgotPassword = false; forgotSuccess = false; forgotEmail = ''">
            Fermer
          </button>
        </div>
      </div>
    ...
    <div class="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 mx-4">
      ...
      <div class="flex mb-6">
        <button :class="['flex-1 py-2 rounded-xl', tab==='login'?'bg-emerald-500 text-white':'bg-gray-100']" @click="tab='login'">
          Connexion
        </button>
        <button :class="['flex-1 py-2 rounded-xl ml-2', tab==='register'?'bg-emerald-500 text-white':'bg-gray-100']" @click="tab='register'">
          Inscription
        </button>
      </div>
      <form class="space-y-3" @submit.prevent="submit">
        <div>
          <input
            ...
            placeholder="Email"
          />
          <p v-if="validationErrors.email" class="text-xs text-red-600 mt-1">
        ...
        <div>
          ...
          />
          <p v-if="validationErrors.password" class="text-xs text-red-600 mt-1">
        ...
        <div v-if="tab==='register'">
          ...
          />
          <p v-if="validationErrors.confirmPassword" class="text-xs text-red-600 mt-1">
            {{ validationErrors.confirmPassword }}
          </p>
        </div>
        
        <p v-if="error && !Object.keys(validationErrors).length" class="text-sm text-red-600">
          {{ error }}
        </p>
        
        <button type="submit" :disabled="loading" class="w-full bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl py-2 font-medium disabled:opacity-50">
          {{ loading ? '...' : tab==='register' ? 'Créer le compte' : 'Se connecter' }}
        </button>
        
        <button
          v-if="tab==='login'"
          type="button"
          class="w-full text-sm text-emerald-600 hover:text-emerald-700 underline"
      ...
      <div class="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 mx-4">
        <div class="text-center">
          ...
          <p class="text-gray-600 mb-4">
            Vous serez déconnecté dans {{ timeoutCountdown }} secondes pour cause d'inactivité.
          </p>
          <div class="flex gap-2">
            <button class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-xl py-2 font-medium" @click="logout">
              Se déconnecter
            </button>
            <button class="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl py-2 font-medium" @click="keepSessionActive">
              Rester connecté
            </button>
          </div>
        </div>
    ...
    <div class="col-span-8 xl:col-span-9 bg-gray-100 h-screen overflow-hidden relative">
      ...
      <div v-else-if="!peer" class="h-full grid place-items-center text-gray-500 bg-gradient-to-br from-gray-50 to-gray-100">
        <div class="text-center px-4">
          <div class="text-3xl font-semibold mb-2 text-gray-700">
            WhatsApp-like Chat
          </div>
          <p class="text-gray-600 mb-6">
            Choisissez un contact à gauche pour démarrer la conversation.
          </p>
          <button class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 underline" @click="logout">
            Se déconnecter
          </button>
        </div>
      </div>
