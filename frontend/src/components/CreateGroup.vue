    ...
    <div class="bg-white rounded-lg w-full max-w-lg mx-4 p-6">
      ...
      <div class="mb-3">
        ...
        <div class="text-xs text-gray-500 mb-2">
          Choisir parmi vos contacts
        </div>
        <div class="max-h-48 overflow-y-auto border rounded-lg p-2 space-y-2">
          <div v-if="contacts.length === 0" class="text-gray-500 text-sm p-2">
          ...
          <label v-for="c in contacts" :key="c._id" class="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg">
            <input
              ...
              class="w-4 h-4"
            />
            <img v-if="c.avatar" :src="c.avatar || ''" class="w-8 h-8 rounded-full object-cover" />
      ...
      <div class="flex items-center justify-end gap-3">
        <button class="px-3 py-1 text-sm text-gray-600" @click="close">
          Annuler
        </button>
        <button :disabled="creating || !canCreate" class="px-3 py-1 bg-emerald-500 text-white rounded-lg disabled:opacity-50 text-sm" @click="createGroup">
