    ...
    <div class="bg-white rounded-lg w-full max-w-2xl mx-4 p-6">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="font-semibold">{{ group.name || 'Groupe' }}</div>
            <div class="text-xs text-gray-500">Créé par {{ group.creator?.username || '...' }}</div>
          </div>
        </div>
        <div>
      ...
      <div class="mb-4">
        ...
        <div class="flex gap-2">
          ...
          <button v-if="canEdit" @click="saveName" class="px-3 py-2 bg-emerald-500 text-white rounded-lg">Enregistrer</button>
        </div>
      </div>
      ...
      <div class="mb-4">
        <div class="flex items-center justify-between mb-2">
          ...
          <div class="text-xs text-gray-500">Rôles affichées</div>
        </div>
        <ul class="space-y-2 max-h-64 overflow-y-auto">
          <li v-for="m in group.members" :key="m.user._id" class="flex items-center gap-3 p-2 border rounded-lg">
            ...
            <div class="flex-1 min-w-0">
              <div class="font-medium truncate">{{ m.user.username }}</div>
              <div class="text-xs text-gray-500">{{ m.user.username }}</div>
            </div>
            <div class="text-xs px-2 py-0.5 rounded-full bg-gray-100">{{ m.role }}</div>
            <div class="flex items-center gap-2">
              <button v-if="isAdmin && m.role !== 'admin'" @click="promote(m.user._id)" class="text-xs px-2 py-1 bg-emerald-500 text-white rounded">Promouvoir</button>
              <button v-if="isAdmin && String(m.user._id) !== String(currentUser._id)" @click="remove(m.user._id)" class="text-xs px-2 py-1 bg-red-500 text-white rounded">Retirer</button>
            </div>
          </li>
        </ul>
      </div>
      ...
      <div class="flex justify-end gap-2">
        <button @click="leave" class="px-3 py-2 bg-gray-100 rounded">Quitter le groupe</button>
      </div>
    </div>
  </div>
</template>
