<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="close">
    <div class="bg-white rounded-lg w-full max-w-md mx-4 p-6">
      ...
      <div class="mb-4">
        <input v-model="q" @input="onInput" placeholder="Rechercher un utilisateur..." class="w-full rounded-lg border px-3 py-2" />
      </div>

      <div v-if="searching" class="py-4 text-center text-gray-500">Recherche...</div>
      <ul v-else class="space-y-2 max-h-64 overflow-y-auto">
        <li v-for="u in results" :key="u._id" class="flex items-center justify-between p-2 border rounded-lg">
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
              <div class="font-medium truncate">{{ u.username || u.email || u._id }}</div>
              <div class="text-xs text-gray-500">{{ u.status || '' }}</div>
            </div>
          </div>
          <button @click="add(u)" :disabled="addingIds[u._id]" class="px-3 py-1 bg-emerald-500 text-white rounded text-sm">{{ addingIds[u._id] ? 'Ajout...' : 'Ajouter' }}</button>
        </li>
      </ul>
